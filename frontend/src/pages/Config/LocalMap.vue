  ...
  <div class="relative">
    <div class="relative q-mb-md input-map">
      <!-- <div class="row items-center q-gutter-sm">
        <q-input dense @input="$emit('onInputLat', $event)" class="col" outlined :value="coordinate[0]" label="Latitude"></q-input>
        <q-input dense @input="$emit('onInputLng')" class="col" outlined :value="coordinate[1]" label="Longitude"></q-input>
      </div> -->
      <div class="q-mt-md">
  ...
  methods: {
    ...
    initialMap() {
      ...
      if(!this.coordinate || !this.coordinate.length) {
        ...
        this.map = L.map('mapContainer').setView(this.coordinate, this.zoom);
      }
  

     if(this.config && this.config.mapbox_access_token) {

        L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=pk.eyJ1Ijoic2lsZWhhZ2UiLCJhIjoiY2wxbnQxejk5MDR2azNrbzEwdDZ2czhhMCJ9.G7SxnM97OX1wz8SAnkMmXg', {
